...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
	...
	if(isset($_POST['s']))
	{
				...
				else
				{
					...
					if ($q_user_tb->rowCount() == 0)
					{
						...
						if($row_website_tb['sms_active'] == "1")
			            {
							...
							{
								$sms = new thsms();
								$result = $sms->send($row_website_tb['sms_user'], $row_website_tb['sms_pass'], $s1_phone, $otp." รหัส OTP สำหรับสมัครสมาชิก ref code ".$ref.".");
								if ($result == "success")
								{
		...
		{
			$s2_bank_code = trim($_POST['s2_bank_code']);
			...
			$s2_id_line = trim($_POST['s2_id_line']);
			if($s2_bank_code != "" && $s2_account_number != "" && $s2_account_name != "" && $s2_register_channel_id != "" && $s2_id_line != "" && $s2_phone != "")
			{
				...
				{
					if($s2_register_channel_id == "7" && $s2_account_aff == "")
					{
						dd_return(false, "กรุณากรอก เบอร์มือถือเพื่อน");
					}
					else
					{
						$_success = true;
						if($s2_register_channel_id == "7" && $s2_account_aff != "")
						{
							if(!preg_match('/^[0-9]+$/', $s2_account_aff))
							{
								$_success = false;
								dd_return(false, "กรุณาตั้ง เบอร์มือถือเพื่อน ด้วยตัวเลข เท่านั้น");
							}
							else if(strlen($s2_account_aff) != 10)
							{
								$_success = false;
								dd_return(false, "กรุณาตั้ง เบอร์มือถือเพื่อน ด้วยตัวเลข 10 ตัวเท่านั้น");
							}
							else if($s2_account_aff == $s2_phone)
							{
								$_success = false;
								dd_return(false, "ห้ามตั้งเบอร์ตัวเอง");
							}
							else
							{
								$q_user_tb_aff = dd_q('SELECT * FROM user_tb WHERE u_user = ?', [
									$s2_account_aff
								]);
								if ($q_user_tb_aff->rowCount() == 0)
								{
									$_success = false;
									dd_return(false, "ไม่มีเบอร์มือถือเพื่อนในระบบ กรุณาตรวจสอบความถูกต้อง");
								}
							}
						}
		...
		else if($step == "s3")
		{
			...
			$ip = trim($_POST['ip']);
			if($s3_phone != "" && $s3_password != "" && $s3_c_password != "" && $s3_u_bank_code != "" && $s3_u_bank_number != "" && $s3_u_fname != "" && $s3_u_refer_id != "" && $ip != "")
			{
						...
						else
						{
							...
							else
							{
								if($s3_password == $s3_c_password)
								{
									...
									if($s3_u_refer_id == "7" && $s3_u_aff != "")
									{
										if(!preg_match('/^[0-9]+$/', $s3_u_aff))
										{
											$_success = false;
											dd_return(false, "กรุณาตั้ง เบอร์มือถือเพื่อน ด้วยตัวเลข เท่านั้น");
										}
										else if(strlen($s3_u_aff) != 10)
										{
											$_success = false;
											dd_return(false, "กรุณาตั้ง เบอร์มือถือเพื่อน ด้วยตัวเลข 10 ตัวเท่านั้น");
										}
										else if($s3_u_aff == $s3_phone)
										{
											$_success = false;
											dd_return(false, "ห้ามตั้งเบอร์ตัวเอง");
										}
